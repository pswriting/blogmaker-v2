...
# ────────────────────────────────────────────────────────────────────
# 설정 (세션 + Streamlit Secrets) — 디스크에 키를 저장하지 않음
# 공개 배포 시 방문자 키가 서로 노출되지 않도록 세션 메모리에만 보관.
# ────────────────────────────────────────────────────────────────────
...
def get_api_key() -> str:
    """우선순위: 세션 입력 → st.secrets. (파일 저장 안 함)"""
...
def get_use_photos() -> bool:
    return st.session_state.get("use_photos", True)
...
def page_settings() -> None:
    ...
    # secrets에 키가 있는지 확인 (배포자가 설정한 경우)
    has_secret = False
    try:
        has_secret = bool(st.secrets.get("ANTHROPIC_API_KEY", ""))  # type: ignore[attr-defined]
    except Exception:
        has_secret = False

    st.subheader("1) Anthropic API 키")
    if has_secret:
        st.success("✅ 서버에 기본 키가 설정돼 있어요. 바로 사용할 수 있어요.")
        st.caption("본인 키로 쓰고 싶으면 아래에 입력하면 그 키가 우선 적용돼요.")
    session_key = st.session_state.get("api_key", "")
    if session_key:
        st.markdown(f"**현재 세션 키:** ✅ 입력됨 (`...{session_key[-4:]}`)")
    elif not has_secret:
        st.markdown("**현재 상태:** ⚠️ 키 미입력")

    st.markdown(
        "키 발급: [console.anthropic.com/settings/keys]"
        "(https://console.anthropic.com/settings/keys) · "
        "입력한 키는 **이 브라우저 세션에만** 보관되고 서버에 저장되지 않아요."
    )
    ...
    with col1:
        if st.button("키 적용", type="primary", use_container_width=True):
            if new_key.startswith("sk-ant-"):
                st.session_state["api_key"] = new_key
                st.success("적용 완료. 다른 탭으로 이동하세요.")
                st.rerun()
            else:
                st.error("올바른 키 형식이 아니에요 (sk-ant-...로 시작해야 함)")
    with col2:
        if st.button("세션 키 지우기", use_container_width=True):
    ...
    st.session_state["use_photos"] = st.checkbox(
        "Openverse 사진 자동 삽입 (무료, API 키 불필요)",
        value=get_use_photos(),
    )
...
def page_blog_writer() -> None:
    ...
    if not api_key:
        st.warning("⚠️ 먼저 **설정** 탭에서 Anthropic API 키를 입력해주세요.")
        return
